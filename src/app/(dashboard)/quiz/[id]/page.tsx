      ...
      {!showQuiz ? (
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {mockCourses.map((course) => {
                ...
                const colors = subjectColors[course.subject] ?? { label: course.subject, badgeClass: 'bg-muted text-muted-foreground border-border' }
                return (
                  <button
                    key={course.id}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold">Quiz généré — {mockQuestions.length} questions</h2>
              <p className="text-sm text-muted-foreground">Théorème de Pythagore · 3ème</p>
            </div>
            <Button variant="outline" size="sm" onClick={() => { setShowQuiz(false); setSelectedCourse(null) }}>
              Nouveau quiz
            </Button>
          </div>
              ...
              <div key={i} className="rounded-2xl border border-border bg-muted/20 p-4 space-y-3">
                ...
                {q.options.length > 0 && (
                  <div className="grid grid-cols-2 gap-1.5 pl-6">
                    {q.options.map((opt, j) => (
                      <div key={j} className="rounded-lg border border-border bg-muted/30 px-3 py-1.5 text-xs text-muted-foreground">
          ...
          <div className="flex gap-3">
            <Button className="flex-1 text-white" style={{ backgroundColor: BRAND }}>
