  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
          ...
          <Badge
            className={`text-xs ${
              limitReached
                ...
                ? 'bg-amber-500/15 text-amber-700 border-amber-500/30 dark:bg-amber-500/20 dark:text-amber-400'
      ...
      <form onSubmit={handleSubmit} className="space-y-6" noValidate>
        ...
        <div
          onClick={
            !isLoading && !limitReached && !topic.trim()
              ? () => showToast('Indiquez le thème du cours que vous souhaitez générer.', 'error')
              : undefined
          }
        >
          <Button
            ...
            className={`w-full h-12 text-base text-white font-bold flex items-center gap-2 ${!topic.trim() && !limitReached ? 'pointer-events-none' : ''}`}
            style={{ backgroundColor: BRAND }}
          >
            {isLoading ? (
              <>
